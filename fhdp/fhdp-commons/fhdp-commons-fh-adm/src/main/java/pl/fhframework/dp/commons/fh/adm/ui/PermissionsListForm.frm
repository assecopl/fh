...
<Form xmlns="http://fh.asseco.com/form/1.0" container="mainForm" formType="STANDARD" modalSize="REGULAR">
    <PanelHeaderFhDP title="{$.adm.permissions.list.header.title}" onClick="close" width="md-12" />
...
<!--        <ReadOnly when="-selectedPermissions.isEmpty()">deletePermissionButton</ReadOnly>-->
    ...
    <Button id="addRoleButton" inlineStyle="margin-left: auto;" horizontalAlign="right" style="default"
            ...
            label="[icon='fas fa-plus']" paddingRight="10"/>
    <PanelGroup>
        <PanelGroup id="rolesPanelGroup" width="lg-3,md-12" label="{$.adm.permissions.list.roles.label}">
            <Table collection="{roles}" iterator="role" selected="{selectedRole}" onRowClick="roleSelected" minRows="3" id="rolesTable">
                <Column label="{$.adm.permissions.list.roles.name}" width="70" value="{role.roleName}" id="roleNameColumn"/>
                <Column id="roleButtonsColumn" width="30" label="[className='d-flex,align-items-center'] [portal='addRoleButton'][/className]">
                    <Button horizontalAlign="left" style="default" width="xs-1" verticalAlign="middle"
                            ...
                            confirmationMsg="{$.adm.roles.list.role.delete.message}" confirmOnEvent="onClick"
                            styleClasses="button-in-operations"/>
                </Column>
            </Table>
        </PanelGroup>
        <PanelGroup id="permissionPanelGroup"
                    width="lg-9,md-12" label="{$.adm.permissions.list.permission.label}"
                    borderVisible="true">
...
<!--                <ColumnPaged id="noColumn" label="{$.adm.permissions.list.permission.column.no}" value="{permission$rowNo}" width="4"/>-->
                <ColumnPaged id="functionNameColumn" label="{$.adm.permissions.list.permission.column.function_name}" value="{permission.functionName}" width="29"/>
                <ColumnPaged id="moduleNameColumn" label="{$.adm.permissions.list.permission.column.module_name}" value="{getModuleName(permission.moduleUUID)}" width="26"/>
                <ColumnPaged id="allowDisallowColumn" label="{$.adm.permissions.list.permission.column.allow_disallow}" width="30" horizontalAlign="center" verticalAlign="middle">
                ...
                <ColumnPaged id="buttonsColumn" width="4" label="[className='d-flex,align-items-center'] [portal='addPermissionButton'][/className]">
                    <Button horizontalAlign="left" style="default" width="xs-1" verticalAlign="middle"
                            ...
                            confirmationMsg="{$.adm.permissions.list.permission.delete.message}" confirmOnEvent="onClick"
                            styleClasses="button-in-operations"/>
                </ColumnPaged>
            </TablePaged>
    ...
    <Row>
        <Spacer width="md-3"/>
<!--        <Button id="addPermissionButton" width="md-1" onClick="addPermission" label="{$.adm.permissions.list.permission.button.add}"/>-->
<!--        <Button id="deletePermissionButton" width="md-2" onClick="deletePermission" label="{$.adm.permissions.list.permission.button.delete}"-->
<!--            confirmationMsg="{$.adm.permissions.list.permission.delete.message}" confirmOnEvent="onClick"/>-->
    </Row>
</Form>
