...
<Form xmlns="http://fh.asseco.com/form/1.0" container="mainForm" hideHeader="true" formType="MODAL" modalSize="XLARGE">
    <PanelHeaderFhDP title="{$.fhdp.adm.functions.select.header.title}" onClick="cancel" width="md-12" />
    ...
    <PanelGroup width="md-1" id="buttonsPanelGroup" wrapperStyle="padding-left: 0px">
        ...
        <Button id="addSelectedFunctionsButton" label="[icon='fa fa-check']" width="100px" onClick="addSelectedFunctionsAsAllowed" hint="{$.fhdp.adm.functions.select.button.allow.hint}"/>
        <Spacer width="md-2" height="20px" id="buttonSpacer2"/>
        <Button id="addDisallowedFunctionsButton" label="[icon='fa fa-times']" width="100px" onClick="addSelectedFunctionsAsDisallowed" hint="{$.fhdp.adm.functions.select.button.disallow.hint}"/>
        <Spacer width="md-2" height="20px" id="buttonSpacer3"/>
        <Button id="removeSelectedFunctionsButton" label="[icon='fa fa-trash']" width="100px" onClick="removeSelectedFunctions" hint="{$.fhdp.adm.functions.select.button.remove.hint}"/>
    </PanelGroup>
    ...
    <Group height="500px" width="md-7" id="selectedFunctionsPanelGroup" wrapperStyle="padding-left: 26px">
        <Table iterator="function" collection="{addedFunctions}" multiselect="true" minRows="1" selected="{selectedFunctionsRight}" id="selectedFunctionsTable">
            <Column label="{$.fhdp.adm.functions.select.function.column.function_name}" value="{function.name}" id="functionNameColumn" width="45"/>
            <Column label="{$.fhdp.adm.functions.select.function.column.module_name}" value="{function.moduleLabel}" id="moduleNameColumn"/>
            <Column label="{$.fhdp.adm.functions.select.function.column.allow_disallow}" horizontalAlign="center" verticalAlign="middle" id="allowDisallowColumn" width="25">
    ...
    <Group>
        <Button id="confirmButton" width="md-3" onClick="confirm" label="{$.fhdp.adm.functions.select.button.confirm}"/>
        <Button id="cancelButton" width="md-3" onClick="cancel" label="{$.fhdp.adm.functions.select.button.cancel}" />
    </Group>
</Form>
