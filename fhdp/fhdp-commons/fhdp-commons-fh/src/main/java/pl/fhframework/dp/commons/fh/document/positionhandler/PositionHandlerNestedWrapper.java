 ...
 */
@Builder
public class PositionHandlerNestedWrapper<POSITION extends Serializable, MODEL extends BaseDocumentFormModel> implements IPositionHandlerBase<MODEL, POSITION> {
    ...
    private final Supplier<String> variantResolver; //pass method variant resolver, typically from AbstractDocumentHandler
    ...
    private final Class nestedElementClass; //element class
    ...
    public void onDeleteItem(POSITION POSITION, String confirmation, String confirmationDeleteMessage,
        ...
        FhUtils.showConfirmDialogYesNo(confirmation, confirmationDeleteMessage, labelButtonYes, () -> {
            ...
            if(variantResolver.get().equals("Correction")) {
                // if we are in correction mode, then we only mark position (row) for delete
    ...
    private POSITION getNewItem() {
        try {
            if(initializer != null) {
                ...
                return positionClass.getDeclaredConstructor(
                        Object.class,
                        Object.class,
                        List.class
                ).newInstance(
                        elementClass.getDeclaredConstructor().newInstance(),
                        elementClass.getDeclaredConstructor().newInstance(),
