 ...
 * <p>
 * Example: {@code <TabContainerFhDP></TabContainerFhDP>}
 */
@TemplateControl(tagName = "fh-tab-container")
@Control(parents = {PanelGroup.class, Column.class, ColumnOptimized.class, Tab.class, Row.class, Form.class, Repeater.class, Group.class}, invalidParents = {Table.class}, canBeDesigned = true)
@DocumentedComponent(category = DocumentedComponent.Category.ARRANGEMENT, documentationExample = true, value = "TabContainerFhDP component which represents a containrt containing a single tabs", icon = "fas fa-window-maximize")
public class TabContainerFhDP extends GroupingComponent<Tab> implements IChangeableByClient, Boundable, CompactLayout, IDesignerEventListener {

    public static final String TYPE_NAME = "TabContainerFhDP";
    ...
    public static final String PORTAL_CONTAINER_ID = "portalContainerId";
    public static final String TABS_SEPARATED = "isTabsSeparated";

    @Getter
    private int activeTabIndex = -1;

    @Getter
    private String portalContainerId;

    @Getter
    private Boolean isTabsSeparated;

    @JsonIgnore
    @Getter
    @Setter
    @XMLProperty(value = PORTAL_CONTAINER_ID) //Powiązanie bindingu z property
    @DesignerXMLProperty(allowedTypes = String.class)
    @DocumentedComponentAttribute(boundable = true, value = "Component's id. This component will be append to additional tab.")
    private ModelBinding portalContainerIdBinding;

    @JsonIgnore
    @Getter
    @Setter
    @XMLProperty(value = TABS_SEPARATED) //Powiązanie bindingu z property
    @DesignerXMLProperty(allowedTypes = boolean.class)
    @DocumentedComponentAttribute(boundable = true, defaultValue = "false", value = "Attribute for seperate tabs.")
    private ModelBinding<Boolean> tabsSeparatedBinding;
    ...
    public TabContainerFhDP(Form form) {
        super(form);
    }
    ...
    protected ElementChanges updateView() {
        ...
        if(portalContainerIdBinding != null){
            BindingResult bindingResult = portalContainerIdBinding.getBindingResult();
            if(bindingResult != null){
                this.portalContainerId = (String)bindingResult.getValue();
                elementChanges.addChange(PORTAL_CONTAINER_ID, this.portalContainerId);
            }
        }

        if(tabsSeparatedBinding != null){
            BindingResult bindingResult = tabsSeparatedBinding.getBindingResult();
            if(bindingResult != null){
                Object value = bindingResult.getValue();
                if (value instanceof String) {
                    this.isTabsSeparated = Boolean.parseBoolean((String) value);
                } else if (value instanceof Boolean) {
                    this.isTabsSeparated = (boolean) value;
                }
            }
        }
        elementChanges.addChange(TABS_SEPARATED, this.isTabsSeparated);
