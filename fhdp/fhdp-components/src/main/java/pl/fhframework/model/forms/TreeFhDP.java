...
@DocumentedComponent(category = DocumentedComponent.Category.TABLE_AND_TREE, documentationExample = true, ignoreFields = {"width"},
        value = "TreeFhDP component is responsible for display certain relation between parent and childs. "
...
public class TreeFhDP extends GroupingComponent<TreeElementFhDP> implements Boundable, CompactLayout, IRepeatableComponentsHolder {
    ...
    @Setter
    @XMLProperty(defaultValue = "true")
    @DocumentedComponentAttribute(value = "Is TreeFhDP processing itself, once it was displayed.", defaultValue = "true")
    ...
    @DocumentedComponentAttribute(value = "Are TreeFhDP node lazy loaded while user expands them.", defaultValue = "false")
    ...
    private TreeElementFhDP templateTreeElement;

    public TreeFhDP(Form form) {
        super(form);
    }
    ...
    public void init() {
        ...
        if (templateTreeElement == null) {
            templateTreeElement = new TreeElementFhDP(getForm());
    ...
    void processComponentsForLevel(int[] parentIndices, int level, IGroupingComponent<TreeElementFhDP> groupingComponent) {
        ...
        BindingResult bindingResult = getForm().getBindingResult(collectionBinding, (Component) groupingComponent); // todo: save to TreeElementFhDP and add binding to processComponentsForLevel parameters.
        if (bindingResult != null && bindingResult.getValue() != null) {
            ...
            if (bindedCollection.size() > groupingComponent.getSubcomponents().size()) {
                for (int i = groupingComponent.getSubcomponents().size(); i < bindedCollection.size(); i++) {
                    TreeElementFhDP clonedTreeElement = (TreeElementFhDP) templateTreeElement.clone().get();
...
//        }
    ...
    ModelBinding replaceModelBinding(ModelBinding binding, int[] indices, int level, TreeElementFhDP clonedTreeElement) {
    ...
    private void deselectAllChild(TreeElementFhDP treeElement) {
    ...
    @Override
    public List<TreeElementFhDP> getSubcomponents() {
        // in design mode expose template TreeFhDP element
        if (getForm().getViewMode() == Form.ViewMode.NORMAL) {
