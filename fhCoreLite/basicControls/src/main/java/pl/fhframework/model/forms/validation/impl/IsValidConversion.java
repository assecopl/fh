...
@Component
public class IsValidConversion implements Validator<BaseInputField> {
    ...
    protected List<ConstraintViolation<BaseInputField>> validate(BaseInputField value, ModelBinding modelBinding) {
        ...
        if (!value.isValidConversion()) {
            addConstraintViolation(violations, value, messageService.getAllBundles().getMessage(Constant.FIELD_IS_INVALID_CONVERSION_KEY));
        }
        return violations;
    }
}
