...
export class ConnectorService {
  ...
  connect(runFunction: any) {
    ...
    if (
      this.target.indexOf('wss://') == 0 ||
      this.target.indexOf('ws://') == 0
    ) {
  ...
  run(command: any, jsonData: any, callback = undefined): any {
    ...
    var requestId = 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(
      /[xy]/g,
      function (c) {
  ...
  decompressData(rawDataString: string, callback: (data: string) => void) {
    if (rawDataString.charAt(0) == '\u0000') {
      ...
      const decompressionString = Pako.ungzip(
        new Uint8Array(rawCompressedData),
        {to: 'string'}
      );
