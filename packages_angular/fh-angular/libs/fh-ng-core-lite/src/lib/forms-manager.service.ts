...
export class FormsManagerService {
  ...
  fireEvent(
    ...
    if (!serviceType && form === false) {
      console.warn(
        '%cForm not found. EventType: %s, formId: %s, componentId: %s',
    ...
    if (!serviceType && form.container === undefined) {
      console.warn(
        '%cForm container not found. EventType: %s, formId: %s, componentId: %s, form: %o',
        'background: #cc0000; color: #FFF',
        eventType,
        formId,
        componentId,
    ...
    deferredEvent.deferred.promise().then(() => {
      ...
      let requestId = this.socketHandler.activeConnector.run(
        SocketOutputCommands.HANDLE_EVENT,
        eventData,
        (requestId, data) => {
    ...
    if (resultArray.length == 1) {
      ...
      for (let singleResult of resultArray) {
        if (singleResult.closeForm)
          mergedResult.closeForm = mergedResult.closeForm.concat(
            singleResult.closeForm
          );
        if (singleResult.openForm)
          mergedResult.openForm = mergedResult.openForm.concat(
            singleResult.openForm
          );
        if (singleResult.changes)
          mergedResult.changes = mergedResult.changes.concat(
            singleResult.changes
          );
