...
export class FhngIteratorDirective<T> implements DoCheck {
  /**
  ...
  constructor(
    ...
    private _template: TemplateRef<RepeaterContext<T>>,
  ...
  private _applyChanges(changes: IterableChanges<T>) {
      ...
      (
        ...
        if (item.previousIndex == null) {
          const view = this._viewContainer.createEmbeddedView(
            this._template,
            new RepeaterContext<T>(null!, this._repeaterDirective!, -1, -1),
          ...
          const tuple = new RecordViewTuple(
            item,
            <EmbeddedViewRef<RepeaterContext<T>>>view
          );
          insertTuples.push(tuple);
        }
      }
    ...
    for (let i = 0, ilen = this._viewContainer.length; i < ilen; i++) {
      const viewRef = <EmbeddedViewRef<RepeaterContext<T>>>(
  ...
  private _perViewChange(
    view: EmbeddedViewRef<RepeaterContext<T>>,
...
class RecordViewTuple<T> {
  constructor(
    public record: any,
    public view: EmbeddedViewRef<RepeaterContext<T>>
  ) {
  }
}
...
export class RepeaterContext<T> {
  constructor(
    public $implicit: T,
    public repeaterDirective: NgIterable<T>,
    public index: number,
    public count: number
  ) {
  }
