...
@Component({
    ...
    providers: [
         ...
         */
        {provide: EwopComponent, useExisting: forwardRef(() => TabContainerComponent)}
    ]
})
export class TabContainerComponent extends GroupingComponentC<TabComponent> implements OnInit, AfterContentInit {
    ...
    @ContentChildren(TabComponent) public subcomponents: QueryList<TabComponent> = new QueryList<TabComponent>();
    ...
    public updateSubcomponent: (subcomponent: TabComponent, index: number) => null;
    ...
    activateDefaultTab(): void {
        if (this.subcomponents.length > 0) {
            if (this.activeTabLabel) {
                ...
                let tab = this.subcomponents.find(c => c.innerId === this.activeTabId);
                if (tab) {
                    tab.active = true;
                    this.boundActiveTabIndex = this.subcomponents.toArray().indexOf(tab);
                }
    ...
    ngOnChanges(changes: SimpleChanges) {
        ...
        if (changes['activeTabLabel'] || changes['activeTabId'] || changes['activeTabIndex']) {
            this.deactivateTabs();
            this.activateDefaultTab();
        }
    }
