...
@Component({
    ...
    providers: [
         ...
         */
        {provide: EwopComponent, useExisting: forwardRef(() => TableComponent)},
        {provide: TableComponentRef, useExisting: forwardRef(() => TableComponent)}]
})
export class TableComponent extends TableComponentRef implements OnInit, AfterViewInit, OnChanges, OnDestroy, AfterContentInit {
    ...
    onExportCsv(): void {
        ...
        if (this.rowsArray && this.rowsArray.length > 0) {
            this.rowsArray[0].childEwopComponents.forEach(component => {
        ...
        this.rowsArray.forEach(row => {
            row.childEwopComponents.forEach(component => {
                if (component instanceof TableCellComponent) {
    ...
    public select(selected: any) {
        if (this.selectedChange?.observers?.length > 0) {
            if (this.multiselect) {
                ...
                const header = this.childEwopComponents.find(element => element instanceof TableHeadRowComponent) as TableHeadRowComponent;
                if (header) {
    ...
    public toggleSelectAll() {
        if (this.selected && this.selected.length > 0 && this.selected.length == this.collection.length) {
            this.selected = [];
