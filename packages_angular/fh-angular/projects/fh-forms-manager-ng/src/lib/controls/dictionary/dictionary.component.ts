...
import {DocumentedComponent, EwopFormatter, IForm} from '@ewop/ng-core';
...
import {KodWartosciSlownika} from "src/app/ewop/slowniki/KodWartosciSlownika";
import {WartoscSlownika} from "src/app/ewop/slowniki/WartoscSlownika";
...
@Component({
    ...
    providers: [
         ...
         */
        {provide: EwopComponent, useExisting: forwardRef(() => DictionaryComponent)}
    ]
})
export class DictionaryComponent extends EwopReactiveInputC implements OnInit, AfterViewInit {
    ...
    public inputFormatter = (x: string | { label: string }) => {
        if (typeof x === 'string') {
            ...
            return this.dictionaryProvider.displayExpresion(x, this.displayExpression)
        }
    };
    ...
    toggleOptionsViewOnClick(eventTarget): void {
        if (eventTarget.classList.contains('ng-arrow-wrapper') && !this.isOpen) {
            this.isOpen = true;
        } else if ((eventTarget.classList.contains('ng-arrow-wrapper') || eventTarget.classList.contains('ng-option')) && this.isOpen) {
            this.isOpen = false;
        }
    }
    ...
    ngOnChanges(changes: SimpleChanges) {
        ...
        if ((changes['dictionaryCode'] || changes['parentCode'] || changes['effectiveDate'])) {
            const d = changes['dictionaryCode'];
            const p = changes['parentCode'];
            const e = changes['effectiveDate'];
            if ((p && p.currentValue != p.previousValue && !p.firstChange)
    ...
    public getData() {
        if (this.dictionaryProvider && this.dictionaryCode) {
            ...
            if (this.dictionaryCode == "SL02100") {
                response = this.dictionaryProvider.dajWartosciLight(this.dictionaryCode, this.parentCode, this.effectiveDate);
            } else {
                response = this.dictionaryProvider.dajWartosci(this.dictionaryCode, this.parentCode, this.effectiveDate)
            }
            response.then(value => {
                this.values = value;
                ...
                if (this.value) {
                    ...
                    if (this.ngselect && (this.value instanceof KodWartosciSlownika || this.inputFormatter(this.values[this.selectedIdx]) != this.inputFormatter(this.value))) {
                        this.ngselect.select(this.values[this.selectedIdx])
                        this.control.setValue(this.values[this.selectedIdx], {onlySelf: true})
                    }
                }
    ...
    public prepareDisplayExpresion() {
        if (this.displayExpression) {
            const de = this.displayExpression.replace(/{/g, "").replace(/}/g, "").replace(new RegExp(/\\/g), "")
    ...
    onScroll({end}, term) {
        ...
        if (end + this.numberOfItemsFromEndBeforeFetchingMore >= this.valuesBuffer.length) {
            this.fetchMore(term);
        }
    }
    ...
    onSearch() {
        this.input$.pipe(
            ...
            switchMap((term: string) => {
                    let data = (this.values.filter(x => term ? this.filterFunction(term, x) : x))
                    return of(data);
                }
            )).subscribe(data => {
    ...
    public fetchMore(term) {
        if (this.recordsCount > this.valuesBuffer.length) {
            ...
            const more = this.values.filter(x => term ? this.filterFunction(term, x) : x).slice(len, this.bufferSize + len);
            this.loading = true;
...
class DictionaryFormatter extends EwopFormatter {
    ...
    fromModel(v: any): string {
        ...
        const value = this.component.values.find(value => value.kodWartosci == v.kodWartosci);
        if (value == null) {
            return {
                ...
                wersjaSlownika: v.oznaczenieWersji ? {oznaczenieWersji: v.oznaczenieWersji} : null,
                nazwa: v.nazwa
            } as any;
        }
        return value;
