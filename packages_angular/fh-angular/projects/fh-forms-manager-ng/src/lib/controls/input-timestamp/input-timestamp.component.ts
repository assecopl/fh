import {
    Component, EventEmitter, forwardRef, Host,
    HostBinding,
    Inject,
    Injector,
    Input,
...
import {EwopDateTimeAdapter} from '../../models/EwopDateTimeAdapter';
...
@DocumentedComponent({
    ...
    value: 'Component responsible for displaying field, where use can set date and time',
    icon: 'fa fa-calendar'
})
@Component({
    selector: 'ewop-input-timestamp',
    ...
    providers: [
         ...
         */
        {provide: EwopComponent, useExisting: forwardRef(() => InputTimestampComponent)},
        NgbInputDatepickerConfig,
        EwopDateTimeAdapter,
        {provide: NgbDateAdapter, useExisting: EwopDateTimeAdapter},
        {provide: NgbTimeAdapter, useExisting: EwopDateTimeAdapter},
        {provide: NgbDateParserFormatter, useExisting: EwopDateTimeAdapter},
    ]
})
export class InputTimestampComponent extends EwopReactiveInputC implements OnInit {
    ...
    constructor(@Inject(LOCALE_ID) private _locale: string, public injector: Injector, public config: NgbInputDatepickerConfig, public calendar: NgbCalendar, public ewopDateTimeAdapter: EwopDateTimeAdapter,
