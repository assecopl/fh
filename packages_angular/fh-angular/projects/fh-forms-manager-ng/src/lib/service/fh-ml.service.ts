...
export class FhMLService {
    ...
    public transform(valueText: string) {
        valueText = (valueText === undefined || valueText === null) ? '' : valueText.toString();

        const unescapedQuotesText = valueText
            .replace(/"/g, '\'')
            .replace(/\\'/g, '\'');
        if (this.needParse(unescapedQuotesText)) {
            return this.sanitizer.sanitize(SecurityContext.HTML, this.sanitizer.bypassSecurityTrustHtml(this.parse(unescapedQuotesText, true)));
        } else {
            return this.sanitizer.sanitize(SecurityContext.HTML, this.sanitizer.bypassSecurityTrustHtml(valueText.replace(/\\'/g, '\'')));
        }
    };
}
