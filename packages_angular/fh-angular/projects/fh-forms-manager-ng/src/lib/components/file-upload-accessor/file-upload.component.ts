...
import {EwopNotificationService} from "@ewop/ng-core";
 ...
 */
@Component({
    selector: 'ewop-file-upload-accessor',
...
export class FileUploadAccessorComponent extends EwopHTMLElementC implements ControlValueAccessor {
    @Input() progress;
    onChange: Function;
    ...
    @HostListener('change', ['$event.target.files']) emitFiles(event: FileList) {
        ...
        // @ts-ignore
        if (this.requiredFilesType(file, this.extensions ? this.extensions.split(',') : null) && this.filesSizeValidator(file)) {
            this.onChange(file);
            this.file = file;
        } else {
            this.onChange(null);
    ...
    constructor(private host: ElementRef<HTMLInputElement>, public injector: Injector,
                public notification: EwopNotificationService, public translate: TranslateService) {
        super(injector, null);
    }
    ...
    public fileSizeValidator(file: File) {
        if (file && this.maxSize) {
            ...
            if (file && file.size >= this.maxSize) {
                //TODO Translate!!
                this.notification.showError(`Plik ${file.name} jest za duży. Maksymalny rozmiar pliku to ${this.maxSize / 1024} KB.`, null, 10000)
                return false
            } else {
                return true;
            }
    ...
    /**
     * This method logic must be similar to EwopValidators.requiredOneFileType
     * @param file
     * @param type
     */
    public requiredFileType(file: File, type: string[]): boolean {
        ...
        if (file && type) {
            ...
            if (existValue == true) {
                ...
                //TODO Translate!!
                this.notification.showError(`Plik tego formatu nie jest obsługiwany. Akceptowalne pliki : ${this.extensions}.`, null, 10000)
                return false
            }
        }
        return true;
