...
import {EwopDateUtils} from "@ewop/ng-core";
 ...
 */
@Injectable()
export class EwopDateAdapter extends NgbDateAdapter<string> {

    public tmpDate: EwopDateTimeStruct = {day: 0, hour: 0, minute: 0, month: 0, second: 0, year: 0};
    public dateFormat: string = EwopDateUtils.FRONTEND_DATE_FORMAT;
    public backendFormat: string = EwopDateUtils.BACKEND_DATE_FORMAT
    ...
    /**
     * Converts a native `string date` to a `EwopDateTimeStruct`.
     */
    public fromModel(date: string | NgbDate | Date): EwopDateTimeStruct | any {
        if (!date) {
            return "";
        }
        ...
        if (date) {
            if (date instanceof NgbDate) {
                // @ts-ignore
                this.tmpDate = this._toEwopDateTimeStruct(date);
                return this.tmpDate;
            } else {
                // @ts-ignore //string and Date
                this.tmpDate = this.parseToEwopDateTime(date);
                return this.tmpDate;
            }
        }
    ...
    /**
     * Converts a `EwopDateTimeStruct` to a native `Date`.
     */
    public toModel(date: EwopDateTimeStruct): any {
        ...
        if (date) {
            ...
            if (this.returnType === "Date") {
                ...
                d = moment(this.formatDateTime(date, this.backendFormat), EwopDateUtils.FRONTEND_DATE_FORMAT).toDate();
            }
        }
        return d;
    }
    ...
    private _toEwopDateTimeStruct(date: EwopDateTimeStruct): EwopDateTimeStruct {
        if (!this.tmpDate) {
            this.todayToTmpDate()
        }
    ...
    public todayToTmpDate() {
        ...
        const date: EwopDateTimeStruct | any = new NgbDate(null, null, null);
        // this.tmpDate = {
     ...
     */
    parse(value: string): EwopDateTimeStruct {
        const result: EwopDateTimeStruct = this.parseToEwopDateTime(value);

        this.tmpDate = result;
        return result;
    }

    format(date: EwopDateTimeStruct): string {

        const result: string = this.formatDateTime(date);
        return result;
    }

    formatDateTime(date: EwopDateTimeStruct, format: string = null): string {
        let result: string = "";
        if (date) {
            this.tmpDate = this._toEwopDateTimeStruct(date);
            ...
            if (this.tmpDate && isNumber(this.tmpDate.year)) {
                result = moment([this.tmpDate.year, this.tmpDate.month - 1, this.tmpDate.day, this.tmpDate.hour, this.tmpDate.minute, this.tmpDate.second, 0]).format(format ? format : this.dateFormat);
            }
        }
        return result;
    }

    private parseToEwopDateTime(value: string | Date): EwopDateTimeStruct {
        let date: EwopDateTimeStruct | any = "";
        if (value) {
...
@Injectable()
export class EwopDateTimeAdapter extends EwopDateAdapter {

    constructor() {
        super();
        this.dateFormat = EwopDateUtils.FRONTEND_DATETIME_FORMAT;
        this.backendFormat = EwopDateUtils.BACKEND_DATETIME_FORMAT;
    }
     ...
     * Converts a `EwopDateTimeStruct` to a native `Date`.
     */
    public toModel(date: EwopDateTimeStruct): string {
...
type EwopDateTimeStruct = NgbDateStruct & NgbTimeStruct;
