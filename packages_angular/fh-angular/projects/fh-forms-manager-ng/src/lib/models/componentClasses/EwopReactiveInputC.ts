...
export class EwopReactiveInputC extends EwopHTMLElementC implements OnInit, OnChanges, IValidatedComponent {
    ...
    public resolveFormControl() {
        ...
        this.control = this.control ? this.control : this.buildFormControl(this.updateOn);

        let groupingControl = this.controlContainer.control as AbstractControl;

        //Add it to FormGroup if its was set on ewop-form.
        if (groupingControl) {
            const controlName = this.name ? this.name.toString().replace(/\[/g, ".").replace(/\]/g, "") : this.innerId;
            const fg = this.name ? controlName.split(".") : controlName.split("!!!!!!!");
            if (fg.length == 1) {
                ...
                const last = fg.pop();
                EwopReactiveInputC.fillFormControlsByPath(groupingControl.get("_model").value, new Array(...fg), groupingControl);
                const targetFormGroup: FormGroup = groupingControl.get(fg.join(".")) as FormGroup;
                if (targetFormGroup) {
...
//Emitujemy wartosc do modelu - działa odpowiednio dla ustwieniem "updateOn" w danej kontrolce.
        ...
        //TODO Same control inside multiple ewopcomponents is not supported.
    ...
    public getSelector() {
        ...
        return selector ? selector.nativeElement.tagName.toLowerCase() : "selector not found";
    }
    ...
    ngOnChanges(changes: SimpleChanges) {
        if (changes['value']) {
            ...
            if (val.previousValue != val.currentValue && this.control != null) {
                this.control.setValue(this.formatter ? this.formatter.fromModel(val.currentValue) : val.currentValue, {onlySelf: true, emitEvent: false});
            }
        }
    ...
    public getValidationLabel(): string {
        return this.validationLabel ? this.validationLabel : (this.label ? this.label : this.id);
    }

    static fillFormControlsByPath(model: any, path: string[], abstractControl: AbstractControl, circularMap: Map<any, FormGroup | FormArray> = new Map<any, FormGroup | FormArray>()) {
        if (path.length == 0) {
            return;
        }
        ...
        if (!(value == null) && !circularMap.has(value)) {
            if (value instanceof Object && !key.startsWith("$") && !(value instanceof Date)) {
                const notExists = abstractControl.get(key) == null;
                if (TypeUtils.isArray(value)) {
                    const fa: FormArray = notExists ? EwopReactiveInputC.createFormArray(value) : abstractControl.get(key) as FormArray;
                    circularMap.set(value, fa);
                    value.forEach((arrayObject, index) => {
                        const fg: FormGroup = notExists ? EwopReactiveInputC.createFormGroup(arrayObject) : fa.get(index.toString()) as FormGroup;
                    ...
                    EwopReactiveInputC.fillFormControlsByPath(value[index], path, fa.get(index), new Map(circularMap));
                } else {
                    const fg: FormGroup = notExists ? EwopReactiveInputC.createFormGroup(value) : abstractControl.get(key) as FormGroup;
                    circularMap.set(value, fg);
                    if (notExists && abstractControl instanceof FormGroup) {
                        abstractControl.addControl(key, fg);
                    }
                    EwopReactiveInputC.fillFormControlsByPath(value, path, fg, new Map(circularMap));
                }
            }
        }
    }
...
export namespace EwopReactiveInputC {
