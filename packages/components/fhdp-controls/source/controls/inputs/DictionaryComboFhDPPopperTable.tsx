...
export const DictionaryComboFhDPPopperTable: React.FC<Props> = (props: Props) => {
  const {clickInPopup, title, columns, hookElementId, isOpen, handleClose, fireChangePopupEvent, recordClick, currentPage, pagesCount, parent, readOnly, position, backgroundColor, translate} = props;
  let {rows} = props;
  ...
  const { styles, attributes, forceUpdate } = usePopper(hookElement, popperElementEx, {placement: placement as Placement });
  ...
  const generateRow = (row: any, columns: Array<{title: string, field: string}>, key: string | number) => {
    return (
      <tr key={`tr-${key}`} onMouseEnter={rowMouseEnter} onMouseLeave={rowMouseLeave} onClick={handleRowClick(row)}>
        {columns.map((column: any, index: number) => <td key={`td-${index}`}  style={styles.td}>{row[column.field]}</td>)}
      </tr>
    )
  }
  ...
  const generatePagination = () => {
    ...
    return (
      <div style={styles.pagination}>
        <button onClick={() => {prevPage()}} style={styles.pageButton} disabled={page <= 0}>{'<'}</button>
        <span>&nbsp;{page + 1}&nbsp;{translate('of')}&nbsp;{totalPages}&nbsp;</span>
        <button onClick={() => {nextPage()}} style={styles.pageButton} disabled={page >= totalPages - 1}>{'>'}</button>
      </div>
    )
  }
  ...
  const generateEmptyRows = () => {
    ...
    return (
      <tr>
        <td style={styles.emptyRows} colSpan={columns.length} onClick={unlockClickInPopup} >{translate('no-data')}</td>
      </tr>
    );
  }
  ...
  const createPortal = () => {
    ...
    return ReactDOM.createPortal((
      <div ref={popperElement} id={`dictionary-combo-popper-${+ new Date()}`} className={'MuiPaper-root'} style={styles.popper} {...attributes.popper} onClick={unlockClickInPopup}>
