...
class FH {
     ...
     */
    public startUseCase(subsystemId: string, useCaseId: string, callback: any) {
        var requestId = this.socketHandler.activeConnector.startUseCase(subsystemId, useCaseId,
            function (requestId, runUCEventHandleResult) {
