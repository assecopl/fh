...
public class SessionLogger implements ISessionLogger{
    public void logUserSessionCreation(Session newSession) {
        if (FhLogger.isDebugEnabled(this.getClass())) {
            if (newSession instanceof UserSession) {
                FhLogger.debug(this.getClass(), String.format("New user session %s.\n" +
                                "User login: %s\n" +
                                "Name: %s\n" +
                                "Surname: %s\n" +
                                "Security roles: %s\n" +
                ...
                FhLogger.debug(this.getClass(), String.format("New session %s.\n" +
                                "Login: %s\n" +
                                "Security roles: %s\n" +
                                "Server address: %s\n",
                        newSession.getDescription().getConversationUniqueId(),
    ...
    public void logSessionState(UserSession session) {
        if (FhLogger.isDebugEnabled(this.getClass())) {
            FhLogger.debug(this.getClass(), "Session State: "
                            + "\n   stackPU {}"
                            + "\n   form: {}",
            ...
            if (!session.getUseCaseRequestContext().getFormsToHide().isEmpty())
                FhLogger.debug(this.getClass(), "Forms to hide: {}", DebugUtils.collectionInfo(session.getUseCaseRequestContext().getFormsToHide()));
            if (!session.getUseCaseRequestContext().getFormsToDisplay().isEmpty())
                FhLogger.debug(this.getClass(), "Forms to show: {}", DebugUtils.collectionInfo(session.getUseCaseRequestContext().getFormsToDisplay()));
