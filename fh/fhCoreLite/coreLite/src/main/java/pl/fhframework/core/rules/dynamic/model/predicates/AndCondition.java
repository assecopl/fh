...
public class AndCondition extends ComplexCondition {
    ...
    protected String getTagName() {
        return AndCondition.TAG_NAME;
    }

    public static RuleElement convertFromBlockly(BlocklyBlock parsedBlock) {
        AndCondition andCondition = new AndCondition();
        andCondition.convertFromBlocklyInternal(parsedBlock);

        return andCondition;
    }

    public static AndCondition of(DefinedCondition... definedConditions) {
        AndCondition andCondition = new AndCondition();

        if (definedConditions != null) {
            Arrays.stream(definedConditions).forEach(definedCondition -> andCondition.getStatements().add(definedCondition));
        }
