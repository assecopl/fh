...
public class AdHocTableColumnOptimizedComponentFactory implements IMultipleIteratorComponentFactory<TableOptimized> {
    ...
    @Override
    public List<FormElement> createComponentsForIterators(TableOptimized myGroupingParent, IRowNumberOffsetSupplier rowNumberOffsetSupplier, int[] indices) {
        if (!initDone) {
            init();
        }
    ...
    private void setUpClonedFormComponent(TableOptimized owningTable, FormElement clone, int[] indices) {
