...
|formatterBinding|ModelBinding|true||Id of formatter which will format object to String. It must be consistent with value of pl.fhframework.formatter.FhFormatter annotation.|
...
|inputType|String|false||Type - text, password|
...
|mask|String|false||Formatter mask|
|maskBinding|ModelBinding|true||Formatter mask.|
|maskDefinition|String|false||Custom symbol to use in mask. First character is always validator, second part defines allowed characters (between square brackets). Multiple values are separated with double pipe (" ". For example: x[123],d[1-8].)|
|maskDynamic|Boolean|false||Should mask characters be added just in time typing.|
|maskInsertMode|Boolean|false||Can mask characters be overriden.|
|maxLength|Integer|false||Maximum length of value|
|maxLengthBinding|ModelBinding|true||Maximum length of value|
