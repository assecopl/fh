...
|componentIds|List|false||Parent component list of ids that group components for validation delimited by "," or * to match all or + to include unassigned custom messages|
...
|level|PresentationStyleEnum|false||Represents level of displayed validation messages, like warning, error, information. Possible values: ok, info, warning, error, blocker|
...
|navigation|boolean|false|true|Defines if field navigation should be handled|
...
|strictLevel|boolean|false|false|Defines if display messages strictly from choosen level|
