...
public class ExpressionTsCodeGenerator extends BaseExpressionCodeGenerator {
    ...
    protected SpelNodeImpl mapMethod(MethodReference node, Supplier<Type> ownerType) {
        ...
        if ("add".equals(name) && isCollection(ownerType)) { // todo: arguments
            // collection
            return new MethodReference(true, "push", node.getEndPosition(), super.getChildren(node).toArray(new SpelNodeImpl[0]));
        } else if ("size".equals(name) && isCollection(ownerType)) {
            // collection
            return new PropertyOrFieldReference(true, "length", node.getEndPosition());
            ...
            if (int.class.isAssignableFrom(ReflectionUtils.getRawClass(paramType))) {
                return new MethodReference(true, "removeAt", node.getEndPosition(), super.getChildren(node).toArray(new SpelNodeImpl[0]));
            }
            return new MethodReference(true, "removeElement", node.getEndPosition(), super.getChildren(node).toArray(new SpelNodeImpl[0]));
        }
        return node;
    }
