...
|extensions|ModelBinding|true||Defines what file extensions are allowed for upload (comma separated list of extensions e.g. .xml,.txt).|
|file|ModelBinding|false||Binding for file.|
|files|ModelBinding|false||Binding for multiple files.|
...
|label|ModelBinding|true||Represents label for created component.|
|labelHidden|boolean|true||Represents label for created component.|
...
|maxSize|long|false|1000000|Maximum allowed size of uploaded file in bytes. The maximum file size depends on the configuration of the server. This information should be obtained from the system administrator.|
|multiple|boolean|true||Is multiple file upload allowed.|
|onDesignerToolboxDrop|ActionBinding|false||If the component is dropped on form edited in designer.|
|onUpload|ActionBinding|false|-|Action which will be executed after file is uploaded.|
...
|required|boolean|false|false|User can define if component is required for Form.|
|style|ModelBinding|true|primary|Determines style of FileUpload button. It is possible to select one of six Bootstrap classes: default, primary, success, info, warning, danger or bind it with variable.|
