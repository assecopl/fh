...
|onClick|ActionBinding|false||If the group is clicked that method will be executed. All components inside that group with actions will be invoked first. The action may or may not be propagated to Group. It depends on used component, so use it with caution.|
